...
For both datasets, adding EMP consistently lowers EMAE on all four baseline models. However, for OE62, the edge-based models gain trivial improvement because a $12\,\AA$ radius already saturates nearly every graph in the dataset; therefore, long-range information processed by EMP is partly redundant.
<figure style="display: flex; flex-direction: column; align-items: center;">
...
F(x',x)
    &= \sum_{t \in T} 
       \Bigl(
         \log P\bigl(y_{t} = x'_{t} \mid x_{\setminus t},\,\theta\bigr)
         \\[-0.3em]
    &\quad\quad
...
$P(y_i = x_i | \psi = 1, x) = P(y_i = x_i | x_{\setminus i})$. This equivalence allows PLL to be computed efficiently within a single inference pass. Therefore, the probability

$P(y_i = x_i | x_{\setminus i}, \theta)$ in PLL(x)

can be redefined using a fixed masking schedule:
...
Using a first‐order Taylor expansion around $\epsilon=0$, the perturbed parameter set $\theta^{*}(\epsilon)$ can be written as
$$
\theta^{*}(\epsilon) = \theta^{*}(0) + \epsilon\,\left.\frac{d\,\theta^{*}(\epsilon)}{d\epsilon}\right|_{\epsilon=0},
$$
where $\left.\frac{d\,\theta^{*}(\epsilon)}{d\epsilon}\right|_{\epsilon=0}$ captures the influence of the datum $z_{m}$ on $\theta^{*}$.

Applying the Implicit Function Theorem and chain rule, we derive:
$$
\begin{equation}
\begin{split}
\left. \frac{d\,\theta^{*}(\epsilon)}{d\epsilon} \right|_{\epsilon=0} \;=\; -\,H^{-1}\,\nabla_{\theta}\mathcal{L}\bigl(z_{m},\,\theta^{*}\bigr).
\end{split}
\end{equation}
$$
...
\begin{equation}
\begin{split}
\left.\frac{d\,f\bigl(\theta^{*}(\epsilon)\bigr)}{d\epsilon}\right|_{\epsilon=0}
&=
\left.\nabla_{\theta}f\bigl(\theta^{*}(\epsilon)\bigr)^{T}
\;\cdot\;\frac{d\,\theta^{*}(\epsilon)}{d\epsilon}\right|_{\epsilon=0}
\\
&=
-\,\nabla_{\theta}f\bigl(\theta^{*}\bigr)^{T}
\,H^{-1}\,
\nabla_{\theta}\mathcal{L}\bigl(z_{m},\,\theta^{*}\bigr)\,.\end{split}
\end{equation}
$$
...
![image](https://github.com/user-attachments/assets/312d7475-4b79-44c2-96c1-a68ff64ccb91)
