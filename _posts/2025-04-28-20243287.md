...
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/img/2025-04-28-20243287/pbc.webp' | relative_url }}" frameborder="0" scrolling="no"/>
...
<figure class="l-page" style="display: flex; justify-content: center; gap: 1rem;">
  ...
  <figcaption class="caption" markdown="1" style="text-align: center; width: 100%;">
...
\begin{equation}
\begin{split}
    \vec{F}(\vec{x_i}) = -\vec{\Delta}V(\vec{x_i}) = -\sum_{\vec{k}}i\vec{k}\hat{V_{\vec{k}}}exp(i\vec{k}^T\vec{x})
\end{split}
\end{equation}
...
- PLMs primarily model the underlying distribution of the training data rather than genuine biological fitness functions <d-cite key="weinstein2022"></d-cite>.
- Training data are often biased towards frequently observed sequences rather than evolutionarily advantageous ones <d-cite key="ding2024"></d-cite>. (Ding & Steinhardt 2024)
- Benchmarks used for model evaluation sometimes overlap with sequences used for training PLM, inflating the resultant model performance <d-cite key="hermann2024"></d-cite>.
...
\begin{equation}
\begin{split}
    F(x^{\prime}, x) = \sum_{t \in T}\logP(y_t = x^{\prime}_t | x_{\ t}, \theta) - \logP(y_t = x_t | x_{\ t}, \theta)
\end{split}
\end{equation}
...
Unlike autoregressive language models, masked language models cannot compute joint likelihood naturally. To address this limitation, Wang & Cho <d-cite key="wang2019"></d-cite> introduced pseudo-log-likelihood (PLL):
\begin{equation}
\begin{split}
    PLL(x) = 1/L\sum_{i=1}^{L} \logP(y_i = x_i | x_{\ i}, \theta)
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
    P(y_i = x_i | x_{\ i}, \theta) = {\alpha + \beta} / \alpha P(y_i = x_i | x, \theta) - \beta / \alpha, 
\end{split}
\end{equation}
...
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/img/2025-04-28-20243287/single_pll.png' | relative_url }}" frameborder="0" scrolling="no"/>
  <figcaption class="caption">
...
Throughout their investigation, the authors primarily compare two prominent PLM models: ProGen2 (autoregressive) and ESM2 (BERT-based). They first explore how model size impacts the correlation between sequence likelihood and zero-shot fitness prediction. For smaller models (8M and 35M parameters), downstream predictive accuracy is largely dependent on the WT sequence likelihood. These smaller models exhibit better performance on sequences closely resembling their training set, as their predictive capabilities are constrained by limited representational power.
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/img/2025-04-28-20243287/small_plms.png' | relative_url }}" frameborder="0" scrolling="no"/>
  <figcaption class="caption">
	  Pseudo Likelihoods vs. Single-Inference Pseudo Likelihoods across different kingdoms.
  </figcaption>
</figure>

**However, how about larger models?** The authors observe a distinct trend for the larger models (150M, 650M, 3B, 15B parameters). These larger models have been overfitted thus over-preferring particular sequences. Consequently, their performance declines as the WT sequence likelihood increases, indicating a tendency toward memorization rather than generalization. Their visual representation reinforces this concept, illustrating how the model’s plausible mutational landscape initially converges toward beneficial mutations but eventually diminishes.
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/html/2025-04-28-20243287/space.html' | relative_url }}" frameborder="0" scrolling="no"/>
  <figcaption class="caption">
	  Pseudo Likelihoods vs. Single-Inference Pseudo Likelihoods across different kingdoms.
  </figcaption>
</figure>
...
Given a training dataset $D$ composed of $N$ samples, $z_i$, the model’s optimal parameters \theta^{*} are obtained by minimizing the loss function $L$: $\theta^{*} \;=\; \argmin_{\theta^{*}}\;J(D, \theta) \;=\; \argmin_{\theta^{*}}\;\frac{1}{N}\sum_{i=1}^{N} \mathcal{L}\bigl(z_i, \, \theta\bigr)$.
...
\end{split}
\end{equation}
Using a first-order Taylor expansion around $\epsilon = 0$, the perturbed parameter set, $theta^{*}(\epsilon) can be expressed as: $theta^{*}(\epsilon) \;=\; theta^{*}(\epsilon) + \epsilon \cdot \frac{d\theta^{*}(\epsilon)}{d\epsilon}\;\left|\right._{\epsilon=0}$, where the derivative $\frac{d\theta^{*}(\epsilon)}{d\epsilon}\;\left|\right._{\epsilon=0}$ captures the influence of the datum $z_m$ on $\theta^{*}$. Applying the Implicit Function Theorem and chain rule, we derive:
\begin{equation}
\begin{split}
...
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/img/2025-04-28-20243287/influence.png' | relative_url }}" frameborder="0" scrolling="no"/>
  <figcaption class="caption">
	  Pseudo Likelihoods vs. Single-Inference Pseudo Likelihoods across different kingdoms.
  </figcaption>
</figure>
...
<figure>
  <img class="invert" style="max-width: 100%; height: auto;" src="{{ 'assets/img/2025-04-28-20243287/power_tail.png' | relative_url }}" frameborder="0" scrolling="no"/>
  <figcaption class="caption">
	  Pseudo Likelihoods vs. Single-Inference Pseudo Likelihoods across different kingdoms.
  </figcaption>
</figure>
